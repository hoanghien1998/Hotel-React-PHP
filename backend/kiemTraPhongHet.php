...
$datenhan = date('Y-m-01');
$datetra =  date('Y-m-t');
$id = $_POST["choseID"];
$tmpDate = $datenhan;
$tmpDate2 = date('Y-m-d', strtotime($datenhan. ' + 1 days'));
$arrayResult = [];
while ($tmpDate <= $datetra) {
    ...
    FROM (SELECT room_type_id,SUM(number_room) AS number_room
        FROM `detailbook`
        WHERE detailbook.check_in <='{$tmpDate}' AND '{$tmpDate}' <= detailbook.check_out AND detailbook.room_type_id = {$id}
    ...
    FROM (SELECT room_type_id,SUM(number_room) AS number_room
        FROM `detailbook`
        WHERE detailbook.check_in <='{$tmpDate2}' AND '{$tmpDate2}' <= detailbook.check_out AND detailbook.room_type_id = {$id}
        GROUP BY room_type_id) AS Detailbook RIGHT JOIN (SELECT roomtype.id,roomtype.name,COUNT(room.id) as number_room
                FROM `room`,`roomtype`
                WHERE room.typeCode=roomtype.id AND roomtype.id = {$id}
                GROUP BY typeCode) AS Roomtype ON Detailbook.room_type_id=Roomtype.id");
    ...
    // var_dump(date('Y-m-d', strtotime($tmpDate. ' + 1 days')));
    $tmpDate = date('Y-m-d', strtotime($tmpDate. ' + 1 days'));
    $tmpDate2 = date('Y-m-d', strtotime($tmpDate2. ' + 1 days'));
