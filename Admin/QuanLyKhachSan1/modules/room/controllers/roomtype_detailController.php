    ...
    if (isset($_POST['save-update-room-type-id'])) {
        ...
        if (isset($_FILES['roomtypedetail_img'])) {
            for ($i = 0; $i < count($_FILES['roomtypedetail_img']['name']); $i++) {
                move_uploaded_file($_FILES['roomtypedetail_img']['tmp_name'][$i], 'C:/xampp/htdocs/Pro_Hotel/Admin/QuanLyKhachSan1/public/images/room/' . $_FILES['roomtypedetail_img']['name'][$i]);
                $data = array(
                    'room_type_id' => $id,
